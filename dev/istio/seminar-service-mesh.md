...
2. Mỗi sidecar khi khởi động sẽ xin istiod một chứng chỉ. Danh tính ghi trong chứng
   ...
   ```
   spiffe://cluster.local/ns/yas/sa/order
...
- Ai gọi `product`? → `cart` (validate giỏ), `order` (checkout), `inventory` và
  `search` (đồng bộ dữ liệu — có code gọi thẳng product API), `recommendation`,
...
metadata:
  ...
  namespace: yas
spec:
  host: "*.yas.svc.cluster.local"   # (3)
  trafficPolicy:
    tls:
      mode: ISTIO_MUTUAL            # (4)
```

- **(1)** PeerAuthentication đặt trong namespace `yas` và **không có `selector`**
...
- **(2)** `STRICT`: mọi pod trong `yas` **từ chối mọi kết nối không mTLS** (phía nhận).
- **(3)** DestinationRule khớp host dạng wildcard — mọi service `*.yas.svc.cluster.local`.
    ...
    - from:
        - source:
            principals:                  # (2) danh tính SPIFFE được phép
              ...
              - cluster.local/ns/yas/sa/inventory      # (3)
              - cluster.local/ns/yas/sa/search         # (3)
              - cluster.local/ns/yas/sa/recommendation
...
- **(2)** Mỗi principal ứng với một service account — xem mục 6. Pod nào dùng SA
  khác (kể cả SA `default` của chính namespace `yas`) đều bị deny.
...
- **(4)** Cho ingress được gọi để đường `api.yas.local.com/product` (trình duyệt →
...
spec:
  hosts:
    - tax                          # (1) tên ngắn
    - tax.yas.svc.cluster.local    #     và tên đầy đủ
  gateways:
...
kubectl get pods -n yas       # các service YAS đang chạy 1/1
```
...
```bash
# Trigger seed
kubectl run curl-seed -n yas --rm -it --restart=Never --image=curlimages/curl:8.8.0 -- \
  curl -X POST -H "Content-Type: application/json" -d '{}' \
  http://sampledata.yas:80/sampledata/storefront/sampledata
# → {"message":"Insert Sample Data successfully!"}

# Xác nhận có data
kubectl run curl-check -n yas --rm -it --restart=Never --image=curlimages/curl:8.8.0 -- \
  curl -s "http://product.yas:80/product/storefront/products?page=0&size=1"
# → totalElements > 0
...
## Bước 3 — Bật sidecar injection cho namespace `yas`
...
Cách đọc tình huống: `kubectl get events -n yas` thấy hàng loạt
...
Web bên ngoài đi vào theo đường: trình duyệt → `ingress-nginx` → service `yas`. Khi
`yas` bật STRICT, ingress **không có sidecar** sẽ gửi plaintext và bị từ chối → web
...
`*.yas.svc.cluster.local`, VirtualService `tax-retry`, 2 AuthorizationPolicy `ALLOW`.
...
**Ca 1 — BỊ CHẶN.** Pod trong `yas` (có sidecar → qua được cửa mTLS) nhưng dùng SA
...
kubectl exec -n yas curl-allowed -- curl -s -o /dev/null -w "HTTP %{http_code}\n" \
  "http://product.yas:80/product/storefront/products?page=0&size=1"   # → HTTP 200
kubectl exec -n yas curl-denied  -- curl -s -o /dev/null -w "HTTP %{http_code}\n" \
  "http://product.yas:80/product/storefront/products?page=0&size=1"   # → HTTP 403
```
...
control plane) → chọn namespace `yas` → Display bật `Traffic`, `Security`,
...
kubectl patch peerauthentication yas-mtls-strict -n yas --type merge \
...
```
<ip-node-worker>  storefront.yas.local.com backoffice.yas.local.com api.yas.local.com identity.yas.local.com
```

→ mở `http://storefront.yas.local.com`. (Cần đủ cả 4 hostname vì trình duyệt còn tự
...
| Kiali topology + giải thích | screenshot graph namespace `yas` với icon khóa mTLS | ✅ |
